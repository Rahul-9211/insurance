...
export default function Header({ transparent = false }: HeaderProps) {
  ...
  const headerStyle = transparent
    ...
    : 'bg-gray-900 shadow-lg py-3'; // Solid everywhere else
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 transition-all duration-300 ${headerStyle}`}>
        ...
        <Link href="/">
          <div className="h-12 w-auto relative">
            <svg width="200" height="40" viewBox="0 0 400 120" xmlns="http://www.w3.org/2000/svg" className="fill-current">
              ...
              {/* Vertical line */}
              <line x1="240" y1="30" x2="240" y2="90" stroke={transparent && !scrolled ? "#111" : "#fff"} strokeWidth="2" />
              
              {/* FINANCE SOLUTIONS text */}
              <text x="250" y="50" fontFamily="Arial, sans-serif" fontSize="28" fontWeight="bold" fill={transparent && !scrolled ? "#111" : "#fff"}>FINANCE</text>
              <text x="250" y="85" fontFamily="Arial, sans-serif" fontSize="28" fontWeight="bold" fill="#a81c1c">SOLUTIONS</text>
              
              {/* Tagline */}
              <text x="250" y="105" fontFamily="Arial, sans-serif" fontSize="12" fill={transparent && !scrolled ? "#111" : "#fff"}>SOLUTIONS YOU CAN BANK ON</text>
            </svg>
          </div>
        </Link>
      </div>
      ...
      <nav className="hidden md:flex items-center space-x-8">
        <Link href="/#about" className="text-white hover:text-gray-300 font-medium">About Us</Link>
